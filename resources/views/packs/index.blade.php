...
@section('content')
    <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">Pengemasan</h4>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item">
                                <a class="text-muted text-decoration-none" href="{{ route('home') }}">Dashboard</a>
                            </li>
                            <li class="breadcrumb-item" aria-current="page">Manage Pengemasan</li>
                        </ol>
                    </nav>
                </div>
                <div class="col-3">
    ...
    <div class="mb-3 d-flex align-items-center gap-2 justify-content-between">
        ...
        <button class="btn btn-primary btn-al-primary"
            data-bs-toggle="modal" data-bs-target="#addPackModal"
        >Tambah</button>
        @include('packs.addmodal',['id' => 'addPackModal'])
    </div>

    <div class="card">
        <div class="card-body">
            <form action="{{route('pack.index')}}" method="GET">
                <div class="row align-items-end mb-3 flex-wrap">
                    <div class="col-md-4 mb-2">
                        <label for="search" class="form-label">Filter Kata</label>
                        <input type="text" class="form-control" placeholder="Cari Kemasan" name="search" value="{{$filter->q ?? ''}}">
                    </div>
                    <div class="col-md-3 mb-2">
                        <label for="field" class="form-label">Urutkan Berdasarkan</label>
                        <select name="field" id="field" class="form-select">
                            @foreach (getModelAttributes('Pack', []) as $atr)
            ...
            <div class="table-responsive">
                <table class="table border text-nowrap mb-0 align-middle">
                    <thead class="text-dark fs-4">
                        <tr>
                            <th>
                                <h6 class="fs-3 fw-semibold mb-0">Kemasan</h6>
                            </th>
                            <th>
                                <h6 class="fs-3 fw-semibold mb-0">Kapasitas</h6>
                            </th>
                            <th>
                                ...
                                <td>
                                    <div class="dropdown dropstart">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <li>
                                                <a href="{{route('pack.index', $pack->stock?->id)}}" class="dropdown-item d-flex align-items-center gap-3"><i
                                                        class="fs-4 ti ti-eye"></i>View</a>
                                            </li>
                                            <li>
                                                <button type="button" class="dropdown-item d-flex align-items-center gap-3"
                                                    data-bs-toggle="modal" data-bs-target="#editModal-{{$pack->id}}"><i
                                                        class="fs-4 ti ti-pencil"></i>Edit Kemasan</button>
                                            </li>
                                            <li>
                                                <button type="button" class="dropdown-item d-flex align-items-center gap-3"
                                                    data-bs-toggle="modal" data-bs-target="#deleteModal-{{$pack->id}}"><i
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form action="{{ route('pack.update', $pack->id) }}" method="POST">
                                                        ...
                                                        <input type="text" name="name" class="form-control mb-2" value="{{ old('name', $pack->name) }}"
                                                        placeholder="Nama Pengemasan">
                                                        <label for="name" class="form-label">Kapasitas</label>
                                                        <input type="number" name="capacity" class="form-control mb-2" value="{{ old('capacity', $pack->capacity) }}"
                                                        ...
                                                        <div class="input-group mb-2">
                                                            ...
                                                            <div style="flex-grow:1">
                                                                <select name="unit_id" id="unit-{{$pack->id}}" class="select2 form-select">
                                                                    <option value="">-- Pilih Satuan --</option>
                                                                    @foreach ($units as $unit)
                                                                        <option value="{{$unit->id}}" {{ $unit->id == old('unit_id', $pack->unit->id) ? 'selected' : '' }}>
                                                                            {{ $unit->name }}
                                                                        </option>
                                                                    @endforeach
                                                                </select>
                                    ...
                                    <!-- Delete Modal -->
                                    <div id="deleteModal-{{$pack->id}}" class="modal fade" tabindex="-1"
                                            ...
                                            <div class="modal-content p-3 modal-filled bg-danger">
                                                <div class="modal-header modal-colored-header text-white">
                                                    <h4 class="modal-title text-white" id="danger-header-modalLabel">
                                                        Yakin ingin menghapus Kemasan ?
                                                    </h4>
                                                    <button type="button" class="btn-close btn-close-white"
                                                        data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body" style="width: fit-content; white-space:normal">
                                                    <h5 class="mt-0 text-white">Kemasan {{$pack->title}} akan dihapus</h5>
                                                    <p class="text-white">Segala data yang berkaitan dengan Kemasan tersebut juga akan dihapus secara permanen.</p>
                                                </div>
